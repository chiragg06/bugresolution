...
export default class WorkItemHelper extends LightningElement {
    @track userName;
    @track userAvatar;
    @track projects = []; 
    @track boards = []; 
    ...
    async fetchBugs(accessToken, jiraCloudId) {
        try {
            ...
            this.bugs = bugsResponse.map(bug => ({
                label: bug.fields.summary,  
                value: String(bug.id)               
            }));
        } catch (error) {
            this.showToast('Error', error.body.message, 'error');
    ...
    handleNext() {
        const compEvent = new CustomEvent('navigate', {
            detail: { bugId: this.selectedBugId }
        });
